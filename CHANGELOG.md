...
- 修复表格单元格文字被正文规则套用首行缩进 2 字符的问题；表格内部段落与正文排版链路隔离，并由表格格式化统一清零首行、左缩进和右缩进。
- 修复 WPS 表格控制符/边界锚点被识别为“空行”的问题；表格内部段落及空的表格结构边界不再进入正文结构识别、空行统计或段落排版计划。
- 修复已勾选“保护嵌入对象”仍会导致内嵌图表被正文固定行距压缩/裁切的问题；图片、图表、Shape、Field、Bookmark 所在段落在保护开启时不再接受正文/标题段落格式，也不会进入文档清理候选。
- 修复安装 Word Formatter 后打开 WPS 表格会额外启动 WPS Writer/WPS Office 进程的问题；宿主识别改为零副作用读取当前 Application，禁止通过 `WpsApplication()`/`EtApplication()` 工厂函数探测宿主，并在非 Writer 宿主中完全跳过 Word Formatter 初始化。

### Changed
- 自动排版不再执行任何自动清理：所有清理项统一标记为 `safeAutoFix: false`；仅允许用户进入“文档清理”后显式确认执行。
- 连续空格、段首/段尾空格、普通空行等仍可被检测并提示，但不会随着“一键排版”自动修改文档文本。
- 新增表格边界、表格单元格缩进隔离、表格空行识别隔离、嵌入对象保护、WPS 宿主隔离与手动清理策略回归测试。
